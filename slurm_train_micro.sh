...
#SBATCH --output=logs/train_micro_%A.out
#SBATCH --error=logs/train_micro_%A.err
#SBATCH --time=4:00:00
#SBATCH --mem=16G
...
head -n $((END_ROW + 2)) PreProcessing/vggsound_train.csv | tail -n $((END_ROW - START_ROW + 1)) > /tmp/vggsound_train_micro_${BATCH_ID}.csv
