...
def get_config():
  ...
  # Match the preprocessing from training (balanced_audioset_base.py)
  config.dataset_configs.num_frames = 32  # Must match training: 32 frames for RGB
  config.dataset_configs.stride = 2  # Stride for RGB sampling
  ...
  config.dataset_configs.test_batch_size = 4  # For single-node inference (was 4 for 4×4 TPU distributed setup)
  ...
  config.model.n_bottlenecks = 4  # Checkpoint has bottleneck=(1,5,768) from n_bottlenecks=4 + classifier='token'
  ...
  config.model.classifier = 'token'  # Checkpoint has bottleneck=(1,5,768) → trained with 'token'
  ...
  config.model.patches.size = [16, 16, 2]  # Verified from checkpoint: RGB kernel shape (2, 16, 16, 3, 768)
  ...
  config.init_from.model_config.model.classifier = 'token'  # Must match checkpoint training
