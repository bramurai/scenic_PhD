...
def get_optimizer(config: ml_collections.ConfigDict) -> optax.GradientTransformation:
  """Constructs the optimizer from the given HParams using optax.
  Args:
    config: Configurations of the optimizer.
  Returns:
    An optax optimizer (GradientTransformation).
  """
  lr = config.lr_configs['base_learning_rate']
  opt_conf = config.optimizer_configs
  ...
  if config.optimizer == 'nesterov':
    return optax.sgd(
        learning_rate=lr,
        momentum=opt_conf.get('momentum', 0.9),
        nesterov=True
    )
  ...
  # No direct optax equivalent for MomentumHP; fallback to sgd
  ...
  if config.optimizer == 'adam':
    if opt_conf.get('weight_decay', 0.0):
      return optax.adamw(
          learning_rate=lr,
          b1=opt_conf.get('beta1', 0.9),
          b2=opt_conf.get('beta2', 0.999),
          eps=opt_conf.get('epsilon', 1e-8),
  ...
  # adabelief not in optax; fallback to adam
...
# --- Legacy flax.optim code for reference ---
